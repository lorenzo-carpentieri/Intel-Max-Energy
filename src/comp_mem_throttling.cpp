...
int main(int argc, char* argv[]){
    ...
    int N_ITERS=1;
    int NUM_RUNS=1; 
    synergy::frequency freq_comp = 1500;
    synergy::frequency freq_mem = 1000; // Set a lower freq for memory bound kernel
    synergy::frequency freq_default=1500;
    std::string kernel1;
    std::string kernel2;
    ...
    for (const auto& [key, value] : params) {
        ...
        if (key == "csv-path") {
            csv_path = s + "/comp_mem_throttling_f";
        }
        else if (key == "runs") {
            NUM_RUNS = std::stoi(s);
        }
    ...
    csv_path += "f" + std::to_string(freq_comp) + "_" + "f" + std::to_string(freq_mem) + ".csv";
    ...
    synergy::frequency core_freq_tile0=freq_default;
    synergy::frequency core_freq_tile1=freq_default;
    ...
    synergy::queue q_total(dev); // Create the synergy queue for the enitre GPU.
