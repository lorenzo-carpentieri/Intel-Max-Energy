...
echo "Running Molecular Dynamics Throttling Benchmark ..."  # Output: app_name
...
mkdir -p ${CSV_DIR}
neigh=16
${BASE_DIR}/bin/mol_dyn --csv-path=${CSV_DIR} --runs=${NUM_RUNS} --freq=${TARGET_FREQ} --iters=1024 --local-size=256 --size=${size} --neigh-count=${neigh}
