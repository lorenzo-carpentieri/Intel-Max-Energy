...
echo "Running Kmeans Throttling Benchmark ..."  # Output: app_name
...
mkdir -p ${CSV_DIR}
${BASE_DIR}/bin/kmeans --csv-path=${CSV_DIR} --runs=${NUM_RUNS} --freq=${TARGET_FREQ} --iters=640 --local-size=256 --size=${size} --num-features=32 --num-clusters=8
