...
echo "Running FTLE Throttling Benchmark ..."  # Output: app_name
...
mkdir -p ${CSV_DIR}
${BASE_DIR}/bin/ftle --csv-path=${CSV_DIR} --runs=${NUM_RUNS} --freq=${TARGET_FREQ} --iters=640 --local-size=256 --size=${size}
