...
echo "Running KNN Throttling Benchmark ..."  # Output: app_name
...
mkdir -p ${CSV_DIR}
nref=$((128*1024))
${BASE_DIR}/bin/knn --csv-path=${CSV_DIR} --runs=${NUM_RUNS} --freq=${TARGET_FREQ} --iters=2 --local-size=256 --size=${size} --nref=${nref}
